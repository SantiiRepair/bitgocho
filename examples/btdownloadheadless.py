...
class HeadlessDisplayer:
    ...
    def display(
        ...
        **kws
        ...
        if (
            self.last_update_time + 0.1 > clock()
        ...
        if statistics is not None:
            ...
            if not self.done:
                self.seedStatus = (
                    "{:d} seen now, plus {:.3f} distributed "
                    "copies".format(statistics.numSeeds, statistics.numCopies)
                )
            else:
                self.seedStatus = (
                    "{:d} seen recently, plus {:.3f} "
                    "distributed copies".format(
                        statistics.numOldSeeds, statistics.numCopies
                    )
                )
            self.peerStatus = "{:d} seen now, {:.1%} done at {:.1f} kB/s" "".format(
                statistics.numPeers,
    ...
    while 1:
        ...
        defaults.append(
            (
                "save_options",
                0,
                "whether to save the current options as the "
        ...
        try:
            ...
            print(
                "error: {}\n".format(e), "run with no args for parameter explanations"
            )
            break
        if not config:
