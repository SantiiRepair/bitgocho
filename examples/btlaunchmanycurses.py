...
from bitgocho.application.number_formats import formatSize, formatIntClock
...
try:
    ...
    print()
    print(
        "This download interface requires the standard Python module "
...
class CursesDisplayer:
    ...
    def _remake_window(self):
        ...
        self.mainwin = curses.newwin(
            self.mainwinh, self.mainwinw + 1, self.mainwiny, self.mainwinx
        )
        self.mainpan = curses.panel.new_panel(self.mainwin)
        self.mainwin.scrollok(0)
        self.mainwin.nodelay(1)

        self.headerwin = curses.newwin(1, self.mainwinw + 1, 1, self.mainwinx)
        self.headerpan = curses.panel.new_panel(self.headerwin)
        self.headerwin.scrollok(0)

        self.totalwin = curses.newwin(
            1, self.mainwinw + 1, self.mainwinh + 1, self.mainwinx
        )
        self.totalpan = curses.panel.new_panel(self.totalwin)
        self.totalwin.scrollok(0)

        self.statuswinh = self.scrh - 4 - self.mainwinh
        self.statuswin = curses.newwin(
            self.statuswinh, self.mainwinw + 1, self.mainwinh + 3, self.mainwinx
        )
    ...
    def display(self, data):
        ...
        if data:
            ...
            self.mainwin.addnstr(
                1, int(self.mainwinw / 2) - 5, "no torrents", 12, curses.A_BOLD
            )
        totalup = 0
        totaldn = 0
        for entry in data:
        ...
        self.totalwin.erase()
        self.totalwin.addnstr(0, self.mainwinw - 27, "Totals:", 7, curses.A_BOLD)
        self.totalwin.addnstr(
            0, self.mainwinw - 20 + (10 - len(totaldn)), totaldn, 10, curses.A_BOLD
        )
        self.totalwin.addnstr(
            0, self.mainwinw - 10 + (10 - len(totalup)), totalup, 10, curses.A_BOLD
        )
...
if __name__ == "__main__":
    ...
    defaults.extend(
        [
            ...
            (
                "saveas_style",
                2,
                "How to name torrent downloads (1 = rename to "
            ...
            (
                "display_path",
                0,
                "whether to display the full path or the torrent "
    ...
    try:
        ...
        if len(sys.argv) < 2:
            print(
                "Usage: btlaunchmanycurses.py <directory> <global options>\n"
