...
from bitgocho.application.number_formats import formatIntClock, formatSize
...
class CursesDisplayer:
    ...
    def _remake_window(self):
        ...
        self.labelwin = curses.newwin(
            self.labelh, self.labelw, self.labely, self.labelx
        )
        self.labelpan = curses.panel.new_panel(self.labelwin)
        self.fieldh, self.fieldw, self.fieldy, self.fieldx = (
            self.labelh,
            (self.scrw - 2) - (self.labelw - 3),
            1,
            self.labelw + 3,
        )
        self.fieldwin = curses.newwin(
            self.fieldh, self.fieldw, self.fieldy, self.fieldx
        )
        self.fieldwin.nodelay(1)
        self.fieldpan = curses.panel.new_panel(self.fieldwin)
        self.spewh, self.speww, self.spewy, self.spewx = (
            self.scrh - self.labelh - 2,
            self.scrw - 3,
            1 + self.labelh,
            2,
        )
        self.spewwin = curses.newwin(self.spewh, self.speww, self.spewy, self.spewx)
        ...
        **kwargs
        ...
        if statistics is not None:
            self.shareRating = "{}  ({:.1f} MB up / {:.1f} MB down)".format(
                ...
                if 0 <= statistics.shareRating <= 100
                else "oo",
            ...
            for i in range(min(self.spewh - 5, len(spew))):
                ...
                if spew[i]["downrate"] > 100:
                    self.spewwin.addnstr(
                        i + 3,
                        38,
                        "{:6.0f} KB/s".format(float(spew[i]["downrate"]) / 1000),
                        11,
                    )
                ...
                self.spewwin.addnstr(
                    i + 3, 58, "{:6.1%}".format(spew[i]["completed"]), 6
                )
