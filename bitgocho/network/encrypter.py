...
class Connection(object):
        ...
        if self.locally_initiated:
            ...
            self.write(
                ...
                + self.encrypter.encrypt(
                    (b"\x00" * 8)
                    + cryptmode  # VC
                    + len(padc).to_bytes(2, "big")  # acceptable crypto modes
                    + padc
                    + b"\x00\x00"  # PadC
                )
    ...
    def read_crypto_pad3(self, s):
        ...
        self.write(
            ("\x00" * 8)
            + cryptmode  # VC
            + len(padd).to_bytes(2, "big")  # encryption mode
            + padd
        )  # PadD
        if ialen:
    ...
    def read_crypto_ia(self, s):
        if DEBUG:
            ...
            if self.buffer:
                self.log.write("r:" + hexlify(self.buffer).decode() + "(buffer)\n")
...
class Encoder(object):
    ...
    def externally_handshaked_connection_made(
        self, connection, options, already_read, encrypted=None
    ):
        if (
            self.paused
            or len(self.connections) >= self.max_connections
