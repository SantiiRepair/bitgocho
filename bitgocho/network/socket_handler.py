...
class SocketHandler(object):
    ...
    def bind(self, port, bind="", reuse=False, ipv6_socket_style=1, upnp=0):
        ...
        if bind:
            ...
            for addr in bind.split(","):
                addrinfos.extend(
                    socket.getaddrinfo(addr, port, socktype, socket.SOCK_STREAM)
                )
        else:
            if self.ipv6_enable:
        ...
        for addrinfo in addrinfos:
            try:
                ...
                if self.ipv6_enable and ipv6_socket_style == 0 and self.servers:
                    raise OSError("blocked port (may require ipv6_binds_v4 to be set)")
                raise e
        if not self.servers:
    ...
    def find_and_bind(
        ...
        portrange = (
            random.sample(range(minport, maxport + 1), nports)
            if randomizer
            else range(minport, minport + nports)
        )
