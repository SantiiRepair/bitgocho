...
defaults = [
    ...
    ("piece_size_pow2", 0, "which power of 2 to set the piece size to (0 = automatic)"),
...
announcelist_details = """announce-list = optional list of redundant/backup tracker URLs, in the
format:
    url[,url...][|url[,url...]...]
...
def make_meta_file(
    loc, url, params=None, flag=None, progress=lambda x: None, progress_percent=True
):
    ...
    info = tree.makeInfo(
        flag=flag, progress=progress, progress_percent=progress_percent, **params
    )
...
def completedir(
    ...
    def subprog(
        update, subtotal=[0], total=sum(tree.size for tree in trees), progress=progress
    ):
        ...
        try:
            ...
            if base not in ignore and base[0] != ".":
                ...
                if "target" in params and params["target"] != "":
                    subparams["target"] = os.path.join(params["target"], base + ext)
                make_meta_file(
                    fname,
                    url,
                    subparams,
