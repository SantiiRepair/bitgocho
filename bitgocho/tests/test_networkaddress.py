...
class AddressRangeTests(unittest.TestCase):
    ...
    def test_addition(self):
        ...
        # Overlapping ranges
        self.assertEqual(
            range1 + range4, AddressRange.from_string("127.0.0.0-127.0.1.128")
        )
        self.assertEqual(
...
class TestAddrList(unittest.TestCase):
    def test_addrlist(self):
        ...
        self.assertListEqual(
            alist.ranges[IPv4], [AddressRange.from_string("127.0.0.1-127.0.0.255")]
        )
        alist.add_ip("127.0.0.0")
        self.assertListEqual(alist.ranges[IPv4], [Subnet.from_string("127.0.0.0/24")])
        alist.add_subnet("127.0.2.0/23")
        self.assertListEqual(
            alist.ranges[IPv4],
            [Subnet.from_string("127.0.0.0/24"), Subnet.from_string("127.0.2.0/23")],
        )
        ...
        self.assertListEqual(alist.ranges[IPv4], [Subnet.from_string("127.0.0.0/22")])
