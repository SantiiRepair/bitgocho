    ...
    def __new__(cls, tracker_url, port, *args, **kwargs):
        ...
        with cls.announcer_lock:
            ...
            if tracker_url not in cls.announcers:
                ...
                announcer.key = random.randint(0, 0xFFFFFFFF).to_bytes(4, "big")
    ...
    def announce(
        ...
        data = b"".join(
            (
                infohash,
                peer_id,
                downloaded.to_bytes(8, "big"),
                left.to_bytes(8, "big"),
                uploaded.to_bytes(8, "big"),
                event.to_bytes(4, "big"),
                self.ip.to_bytes(4, "big"),
                self.key,
                num_want.to_bytes(4, "big", signed=True),
        ...
        return Response(
            interval=int.from_bytes(rawdata[:4], "big"),
...
class HTTPAnnouncer(Announcer):
    ...
    Additional otherwise undocumented bitgocho extensions implemented:
    ...
    def set_options(
        ...
        self.peer_options = [("compact", int(compact))] + [("no_peer_id", 1)][
            : (no_peer_id and not compact)
        ]

        self.crypto_options = [
            ("supportcrypto", int(supportcrypto)),
    ...
    def _redirect(self, new_url, old_query):
        ...
        if self._redirects >= self.MAX_REDIRECTS:
            raise IOError(
                ("http error", 500, "Internal Server Error: Redirect Recursion")
            )
        ...
        new_query = urllib.parse.urlencode(
            (key, val)
            for key, val in urllib.parse.parse_qsl(query)
            if val != oq_dict.get(key, "")
        )
        tracker_url = urllib.parse.urlunsplit((scheme, netloc, path, new_query, frag))

        with self.announcer_lock:
            # It's possible to have the same redirect happen
            if tracker_url in self.announcers and self.announcers[tracker_url] == self:
                return
            self._redirects += 1
    ...
    def announce(
        ...
        if snoop:
            ...
            stats = [("uploaded", uploaded), ("downloaded", downloaded), ("left", left)]
            trackercomm = (
...
def urls_to_announcers(trackerlist, *args, **kwargs):
    ...
    return [[Announcer(url, *args, **kwargs) for url in tier] for tier in trackerlist]
