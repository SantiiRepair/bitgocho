...
class LaunchMany:
    def __init__(self, config, Output):
        try:
            ...
            self.ratelimiter = RateLimiter(
                self.rawserver.add_task, config["upload_unit_size"]
            )
            ...
            for hash in self.torrent_list:
                self.Output.message(
                    'dropped "{}"'.format(self.torrent_cache[hash]["path"])
                )
    ...
    def saveAs(self, hash, name, saveas, isdir):
        ...
        if isdir and not os.path.isdir(saveas):
            try:
                ...
                raise OSError(
                    "couldn't create directory for {} ({})".format(x["path"], saveas)
                )
        return saveas
    ...
    def died(self, hash):
        if hash in self.torrent_cache:
            self.Output.message('DIED: "{}"'.format(self.torrent_cache[hash]["path"]))
