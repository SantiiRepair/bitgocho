...
class SingleDownload:
    def __init__(self, downloader, url):
        ...
        self.baseurl = url
        try:
            (scheme, self.netloc, path, pars, query, _) = urllib.parse.urlparse(url)
        except ValueError:
        ...
        if query:
            self.seedurl += query + "&"
        self.seedurl += "info_hash=" + urllib.parse.quote(self.downloader.infohash)
    ...
    def _fulfill_requests(self):
        ...
        while self.requests:
            ...
            if not self.downloader.storage.piece_came_in(
                self.index, begin, self.received_data[start : start + length]
            ):
                success = False
                break
            start += length
