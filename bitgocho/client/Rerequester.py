...
def check_peers(message):
    ...
    if isinstance(peers, list):
        for peer in peers:
            ...
            if "peer id" in peer:
                check_type(peer.get("peer id"), bytes, pred=lambda x: len(x) != 20)
    ...
    check_type(message.get("min interval", 1), int, pred=lambda x: x <= 0)
...
class Rerequester:
    def __init__(
        ...
        self.announcers = [random.sample(tier, len(tier)) for tier in announcers]
    ...
    def rerequest(self, announce_kwargs, callback):
        ...
        if not self.lock.isfinished():
            ...
            return
        self.lock.reset()
        rq = threading.Thread(target=self._rerequest, args=[announce_kwargs, callback])
        rq.setDaemon(False)
        rq.start()
        ...
        try:
            ...
            for tier in self.announcers:
                ...
                for i, announcer in enumerate(tier):
                    if self.rerequest_single(announcer, announce_kwargs, callback):
    ...
    def rerequest_single(self, announcer, kwargs, callback):
        code = self.lock.set()
        rq = threading.Thread(
            target=self._rerequest_single, args=[announcer, kwargs, code, callback]
        )
        rq.setDaemon(False)
        rq.start()
        self.lock.wait()
    ...
    def _rerequest_single(self, announcer, kwargs, code, callback):
        try:
            ...
            try:
                response = announcer.announce(self.infohash, self.myid, **kwargs)
                check_peers(response)
            except (IOError, OSError) as e:
                if self.lock.trip(code):
                    self.errorcodes[
                        "troublecode"
                    ] = "Problem connecting to " "tracker - " + str(e)
                    self.lock.unwait(code)
                return
            ...
            if "failure reason" in response:
                if self.lock.trip(code):
                    self.errorcodes["rejected"] = (
                        self.rejectedmessage + response["failure reason"]
                    )
                    self.lock.unwait(code)
                return
    ...
    def postrequest(self, r, callback):
        ...
        for i, x in enumerate(p):
            peers.append(((x["ip"].strip(), x["port"]), x.get("peer id", 0), cflags[i]))
