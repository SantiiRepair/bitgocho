...
defaults = [
    ...
    ("metafile", "", "file the server response was stored in, alternative to url"),
    ("url", "", "url to get file from, alternative to metafile"),
    (
        "crypto_allowed",
        int(CRYPTO_OK),
...
def parse_params(params, presets={}):
    ...
    if args:
        if config["metafile"] or config["url"]:
            raise ValueError(
                "must have metafile or url as arg or " "parameter, not both"
            )
...
class BT1Download:
    def __init__(
        ...
        if appdataobj:
            ...
            self.appdataobj.deleteOldCacheData(
                config["expire_cache_data"], [self.infohash]
            )
        ...
        self.choker = Choker(
            config, rawserver.add_task, self.picker, self.finflag.is_set
        )
    ...
    def saveAs(self, filefunc, pathfunc=None):
        try:
            ...
            if "length" in info:
                ...
                fname = filefunc(
                    info["name"], file_length, self.config["saveas"], False
                )
                if fname is None:
                    return None
                make(fname)
                ...
                fname = filefunc(info["name"], file_length, self.config["saveas"], True)
                if fname is None:
                    return None
                        ...
                        if not existing:
                            ...
                            if os.path.exists(fname) and not os.path.isdir(fname):
                                if fname[-8:] == ".torrent":
                                    fname = fname[:-8]
                                if os.path.exists(fname) and not os.path.isdir(fname):
                                    self.errorfunc("Can't create dir - " + info["name"])
                                    return None
                make(fname, True)
    ...
    def _data_flunked(self, amount, index):
        ...
        if not self.doneflag.is_set():
            self.errorfunc(
                "piece {:d} failed hash check, re-downloading it" "".format(index)
            )
    ...
    def startEngine(self, ratelimiter=None, statusfunc=None):
        ...
        self.upmeasure = Measure(
            self.config["max_rate_period"], self.config["upload_rate_fudge"]
        )
        ...
        if ratelimiter:
            ...
            self.ratelimiter = RateLimiter(
                self.rawserver.add_task, self.config["upload_unit_size"], self.setConns
            )
        ...
        self.downloader = Downloader(
            self.storagewrapper,
            self.picker,
            self.config["request_backlog"],
            self.config["max_rate_period"],
            self.len_pieces,
            self.config["download_slice_size"],
            self._received_data,
            self.config["snub_time"],
    ...
    def startRerequester(self, force_rapid_update=False):
        tracker_urls = self.metainfo.get("announce-list", [[self.metainfo["announce"]]])
        kwargs = {
            "port": self.port,
    ...
    def _init_stats(self):
        ...
        if "files" in self.metainfo["info"]:
            self.statistics.set_dirstats(
                self.files, self.metainfo["info"]["piece length"]
            )
        if self.config["spew"]:
            self.spewflag.set()
