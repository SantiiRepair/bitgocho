...
class BadDataGuard:
    ...
    def failed(self, index, bump=False):
        ...
        if len(self.stats.bad) >= 3 and len(self.stats.bad) > int(
        ...
        if lost_interests:
            for d in self.downloader.downloads:
                if d.active_requests or not d.interested:
                    continue
                if (
                    d.example_interest is not None
                    and self.downloader.storage.do_I_have_requests(d.example_interest)
                ):
                    continue
...
class Downloader:
    def __init__(
        ...
        self.totalmeasure = Measure(
            max_rate_period * storage.piece_length / storage.request_size
        )
