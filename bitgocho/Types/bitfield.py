...
CHARBITMAP = [
    tuple(bool((integer << nbits) & 0x80) for nbits in range(8))
...
class Bitfield(list):
    ...
    def __init__(self, length=None, bitstring=None, copyfrom=None, val=False):
        ...
        if length is None:
            raise ValueError(
                "length must be provided unless copying from " "another array"
            )
    ...
    def __bytes__(self):
        ...
        return bytes(BITCHARMAP[tuple(bits[x : x + 8])] for x in range(0, len(bits), 8))
