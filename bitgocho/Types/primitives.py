...
class FixedLengthBytes(bytes):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(val) != cls.length:
            raise ValueError("invalid length for {}: {!r}".format(cls.__name__, val))
        return val
...
class UnsignedInt(int):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if val < 0:
            raise OverflowError("can't convert negative int to {}".format(cls.__name__))
        if cls.bits is not None and val.bit_length() > cls.bits:
            raise OverflowError(
                "too large a value for {}: {!s}".format(cls.__name__, val)
            )
        return val
