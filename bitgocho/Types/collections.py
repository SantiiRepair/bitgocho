...
def normalize(arg, targettype, targetmap):
    ...
    if targettype is not None and argtype is not targettype:
        if isinstance(targettype, tuple) and isinstance(arg, collections.Iterable):
            return tuple(
                normalize(subarg, subtype, targetmap)
    ...
    def _normalized(method):
        ...
        def new_method(self, *args):
            ...
            for arg in args:
                ...
                try:
                    arg = list(normalize(sub, self.valtype, self.valmap) for sub in arg)
                except (TypeError, ValueError):
                    raise TypeError(
                        "Values must be coercible to type '{}'"
...
class SplitList(TypedList):
    ...
    def __init__(self, *args):
        ...
        if len(args) > 1:
            raise TypeError(
                "{}() takes at most 1 argument ({:d} given)"
...
class TypedDict(CopyMixin, dict):
    ...
    def __setitem__(self, key, val):
        ...
        if (
            self.typemap is not None
            and key in self.typemap
            and type(val) is not self.typemap[key]
        ):
    ...
    def update(self, *args, **kwargs):
        ...
        if nargs > 1:
            raise TypeError(
                "update expected at most 1 arguments, got " "{:d}".format(nargs)
            )
        if args:
            arg = args[0]
...
class DictSet(TypedDict, collections.MutableSet):
    ...
    def _normalize_seq(cls, seq):
        ...
        if isinstance(seq, collections.Iterable):
            ...
            # Sequence of pairs
            if (
                isinstance(vals[0], collections.Sequence)
