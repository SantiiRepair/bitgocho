...
def check_info(info):
    ...
    check_type(
        info.get("pieces"), bytes, berr + "bad pieces key", lambda x: len(x) % 20 != 0
    )

    check_type(
        info.get("piece length"), int, berr + "illegal piece length", lambda x: x <= 0
    )
    ...
    if "length" in info:
        check_type(info["length"], int, berr + "bad length", lambda x: x < 0)
    else:
        ...
        for finfo in files:
            ...
            check_type(finfo.get("length"), int, berr + "bad length", lambda x: x < 0)
            ...
            for directory in path:
                ...
                if not valid_name.match(directory):
                    raise ValueError(
                        "path {} disallowed for security reasons" "".format(directory)
                    )
...
class PieceHasher(object):
    ...
    def update(self, data, progress=lambda x: None):
        ...
        if remainder:
            ...
            hashes = [
                self._hashtype(remainder[i : i + self.pieceLength])
...
class Info(TypedDict):  # pylint: disable=R0904
    ...
    def __init__(
        self, name, size=None, progress=lambda x: None, progress_percent=False, **params
    ):
        """
        Parameters
        ...
        if "files" in params:
            ...
            self["length"] = sum(entry["length"] for entry in self._get("files"))
        elif "length" in params:
            self["length"] = params["length"]
            self["files"] = [{"path": [self["name"]], "length": self._get("length")}]
        else:
            self["files"] = []
            self["length"] = size
        ...
        if "pieces" in params:
            ...
            self.hasher.pieces = [pieces[i : i + 20] for i in range(0, len(pieces), 20)]
            self.totalhashed = self._get("length")
        elif size:
            # BitTorrent/bitgocho have traditionally allowed this parameter
