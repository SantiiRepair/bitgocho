...
defaults = [
    ...
    ("log_nat_checks", 0, "whether to add entries to the log for nat-check results"),
    (
    ...
    (
        "keep_dead",
        0,
        "keep dead torrents after they expire (so they still "
    ...
    (
        "dedicated_seed_id",
        "",
        "allows tracker to monitor dedicated seed(s) and flag torrents as " "seeded",
    ),
...
class Tracker(object):
    def __init__(self, config, rawserver):
        ...
        if favicon:
            try:
                ...
                print(
                    "**warning** specified favicon file -- %s -- does not "
                    "exist." % favicon
                )
        ...
        if CHECK_PEER_ID_ENCRYPTED and not CRYPTO_OK:
            print(
                "**warning** crypto library not installed, cannot "
        ...
        self.becache = defaultdict(
            lambda: [({}, {}) for _ in range(3 if config["compact_reqd"] else 5)]
        )
        ...
        for infohash, peers in self.downloads.items():
            ...
            for peerid, peer in list(peers.items()):
                ...
                if (
                    self.allowed_IPs
                    and ip not in self.allowed_IPs
                    or self.banned_IPs
                    and ip in self.banned_IPs
                ):
                    del peers[peerid]
                    continue
                if not peer["left"]:
        ...
        self.times = {
            dl: {sub: 0 for sub in subs} for dl, subs in self.downloads.items()
        }
        ...
        if config["logfile"] and config["logfile"] != "-":
            try:
                ...
                print(
                    "**warning** could not redirect stdout to log file:",
                    sys.exc_info()[0],
                )
        ...
        if config["allowed_list"]:
            if config["allowed_dir"]:
                print(
                    "**warning** allowed_dir and allowed_list options "
    ...
    def aggregate_senddata(self, query):
        threading.Thread(
            target=self.aggregate_forward.forward_query,
        ...
        try:
            ...
            if red:
                return (
                    302,
                    "Found",
                    {"Content-Type": "text/html", "Location": red},
            ...
            if self.config["allowed_dir"]:
                if self.show_names:
                    names = [
                        (self.allowed[infohash]["name"], infohash)
                ...
                for name, infohash in names:
                    ...
                    if self.config["allowed_dir"] and self.show_names:
                        ...
                        s.write(
                            ...
                            '<td align="right"><code>%i</code></td>'
                            '<td align="right"><code>%i</code></td>'
                            "</tr>\n" % (hexlify(infohash).decode(), c, d, n)
                        )
                if self.config["allowed_dir"] and self.show_names:
                    s.write(
                        '<tr><td align="right" colspan="2">%i files</td>'
        ...
        if "info_hash" in paramslist:
            if self.config["scrape_allowed"] not in ["specific", "full"]:
                return (
                    ...
                    bencode(
                        {
                            "failure reason": "specific scrape function "
            ...
            if self.config["scrape_allowed"] != "full":
                return (
                    ...
                    bencode(
                        {
                            "failure reason": "full scrape function is "
    ...
    def check_allowed(self, infohash, paramslist):
        if self.aggregator_key is not None and not (
            ...
            return (
                ...
                bencode(
                    {
                        "failure reason": "Requested download is not "
            ...
            if self.config["allowed_controls"]:
                if "failure reason" in self.allowed[infohash]:
                    return (
                        ...
                        {"Content-Type": "text/plain", "Pragma": "no-cache"},
                        bencode(
                            {"failure reason": self.allowed[infohash]["failure reason"]}
                        ),
                    )
        ...
        if "tracker" in paramslist:
            ...
            if (
                self.config["multitracker_allowed"] == "none"
                or paramslist["peer_id"][0] == self.trackerid
            ):
                return (
                    200,
                    "Not Authorized",
            ...
            if (
                self.config["multitracker_allowed"] == "autodetect"
    ...
    def add_data(self, infohash, event, ip, paramslist):
        ...
        gip = params("ip")
        override = is_valid_ip(gip) and (islocal or not self.only_local_override_ip)
        real_ip = gip if override else ip

        rsize = min(int(params("numwant", self.response_size)), self.response_size)
    ...
    def peerlist(
        self, infohash, stopped, tracker, is_seed, return_type, rsize, supportcrypto
    ):
        ...
        if cache and (
            not cache[1]
            or is_seed
            and len(cache[1]) < rsize
            or len(cache[1]) < l_get_size
            or cache[0] + self.config["min_time_between_cache_refreshes"]
            < self.cachetime
        ):
            cache = None
        if cache is None:
    ...
    def get(self, connection, path, headers):
        ...
        try:
            ...
            if path not in ("announce", "announce.php", "tracker.php/announce"):
                return (
                    404,
                    "Not Found",
                    {"Content-Type": "text/plain", "Pragma": "no-cache"},
        ...
        if self.dedicated_seed_id:
            if (
                params("seed_id") == self.dedicated_seed_id
                and int(params("left", 1)) == 0
            ):
    ...
    def natchecklog(self, peerid, ip, port, result):
        ...
        print(
            '%s - %s [%02d/%3s/%04d:%02d:%02d:%02d] "!natcheck-%s:%i" %i '
            "0 - -"
            % (
                ip,
                urllib.parse.quote(peerid),
                day,
                months[month],
                year,
                hour,
    ...
    def parse_allowed(self):
        ...
        if self.config["allowed_dir"]:
            try:
                ...
                print("**warning** Error updating allowed torrents. " "Reparsing.")
                r = parsedir(self.config["allowed_dir"], {}, {}, set(), [".torrent"])

            (self.allowed, self.allowed_dir_files, self.allowed_dir_blocked, added) = r[
                :-1
            ]
    ...
    def expire_downloaders(self):
        ...
        if not self.keep_dead:
            for infohash, peers in list(self.downloads.items()):
                if len(peers) == 0 and (
                    self.allowed is None or infohash not in self.allowed
                ):
                    del self.times[infohash]
                    del self.downloads[infohash]
                    del self.seedcount[infohash]
        self.rawserver.add_task(
            self.expire_downloaders, self.timeout_downloaders_interval
        )
