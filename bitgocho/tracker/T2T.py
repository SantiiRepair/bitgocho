...
class T2TConnection:
    def __init__(
        ...
        if self.isactive():
            rawserver.add_task(
                self.refresh, random.randrange(int(self.interval / 10), self.interval)
            )
    ...
    def callback(self):
        ...
        if self.lastsuccessful:
            ...
            if self.rerequester.announce_interval > (3 * self.interval):
                ...
                self.peers = int(
                    self.peers * (self.rerequester.announce_interval / self.interval)
                )
            self.operatinginterval = self.rerequester.announce_interval
            if DEBUG:
                print(
                    "{} with info_hash={} returned {:d} peers".format(
                        self.tracker,
                        urllib.parse.quote(self.hash),
    ...
    def errorfunc(self, r):
        ...
        if DEBUG:
            print(
                "{} with info_hash={} gives error: '{}'".format(
        ...
        if self.errors >= 3:  # three or more errors in a row
            ...
            if DEBUG:
                print(
                    " -- lengthening interval to {} seconds".format(
