...
class PeerID(object):
    ...
    def __init__(self):
        initial, version = bitgocho.version_short.split("-")
    ...
    def reset(self):
        ...
        toc3 = 0 if toc2 >= 1000 else countwhile(lambda x: tic == int(time.time() * 10))

        x += "{!r}/{}/{}/{}/{}/{}".format(
            time.time(), time.time(), toc1, toc2, toc3, os.getpid()
        ).encode()
