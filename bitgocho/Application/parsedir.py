...
def parsedir(
    ...
    removed_files = {
        path: files[path]
        for path in files
        if path not in new_files or files[path][0] != new_files[path][0]
    }

    # Missing files are removed
    removed = {filehash: parsed[filehash] for _, filehash in removed_files.values()}

    # unchanged_files = files \ removed_files
    unchanged_files = {path: files[path] for path in files if path not in removed_files}
    ...
    new_parsed = {
        infohash: parsed[infohash] for _, infohash in unchanged_files.values()
    }
        ...
        try:
            ...
            if infohash not in new_parsed:
                ...
                if path not in blocked:
                    errfunc(
                        "**warning** {} is a duplicate torrent for {}"
...
def get_files(directory, exts=(".torrent",)):
    ...
    subs = (candidate for candidate in os.listdir(directory) if candidate[0] != ".")
    ...
    for sub in subs:
        ...
        if extmatches:
            files[loc] = [(int(os.path.getmtime(loc)), os.path.getsize(loc)), 0]
...
def parse_torrent(path, return_metainfo=False):
    ...
    torrentinfo = {
        ...
        "length": info["length"]
        if single
        else sum(li["length"] for li in info["files"] if "length" in li),
    }
