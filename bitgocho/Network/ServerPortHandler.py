...
class SingleRawServer(object):
    ...
    def _external_connection_made(self, c, options, already_read, encrypted=None):
        if self.running:
    ...
    def add_task(self, func, delay=0, tid=None):
        if not self.finished:
            self.rawserver.add_task(func, delay, self.info_hash if tid is None else tid)
        ...
        for k in srs:
            if self.encrypter.test_skey(s, k):
                srs[k]._external_connection_made(
                    self.connection, None, self.buffer, encrypted=self.encrypter
                )
                return True
        return None
