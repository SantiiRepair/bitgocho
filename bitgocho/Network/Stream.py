...
def _url_sig(url):
    ...
    port = (
        int(portstr[0]) if portstr else defaults[scheme] if scheme in defaults else None
    )
    if port is None:
        return None
...
class ShareUDP(SharedStream):
    ...
    def _connect(self):
        ...
        if response[:4] in (self.ERROR, self.ERROR_ALT) and response[4:8] == self.tx_id:
    ...
    def request(self, msg):
        ...
        with self.lock:
            ...
            if (
                response[:4] in (self.ERROR, self.ERROR_ALT)
                and response[4:8] == self.tx_id
            ):
        ...
        with self.lock:
            ...
            try:
                self.connection.request(
                    "GET",
                    msg,
                    headers={"User-Agent": VERSION, "Accept-Encoding": "gzip"},
                )
    ...
    class ShareHTTPS(ShareHTTP):
        ...
        def _connect(self):
            ...
            self.connection = HTTPSConnection(
                *self.sig[1:], timeout=30, context=self.SSLCONTEXT
            )
...
def geturl(url, max_redirects=10):
    ...
    for _ in range(max_redirects):
        ...
        query = urllib.parse.urlunsplit(("", "") + urllib.parse.urlsplit(url)[2:])
