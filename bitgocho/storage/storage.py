...
class storage:
            ...
            try:
                ...
                if DEBUG:
                    print(
                        "{} modified: ({}) != ({}) ?".format(
                            fname,
                            time.strftime("%x %X", time.localtime(self.mtimes[fname])),
                            time.strftime(
                                "%x %X", time.localtime(os.path.getmtime(fname))
                            ),
                        )
                    )
    ...
    def _get_disabled_ranges(self, fileidx):
        ...
        pieces = list(
            range(
                int(start / self.piece_length), int((end - 1) / self.piece_length) + 1
            )
        )
        offset = 0
        disabled_files = []
        if len(pieces) == 1:
            if (
                start % self.piece_length == 0 and end % self.piece_length == 0
            ...
            if start % self.piece_length != 0:
                ...
                offset = length
                piece = pieces.pop(0)
                update_pieces.append(
                    (piece, start - (piece * self.piece_length), length)
                )
            else:
                working_range_b = []
    ...
    def pickle(self):
        ...
        for i, (fname, size) in enumerate(self.files):
            ...
            if not self.disabled[i]:
                files.extend([i, os.path.getsize(fname), int(os.path.getmtime(fname))])
            else:
            ...
            for i, (fname, size) in enumerate(self.files):
                ...
                if self.disabled[i]:
                    for fname, start, end in self._get_disabled_ranges(i)[2]:
                        ...
                        if f1 not in pfiles or changed(
                            pfiles[f1], os.path.getsize(fname), os.path.getmtime(fname)
                        ):
                            if DEBUG:
                ...
                if i not in files or changed(
                    files[i], os.path.getsize(fname), os.path.getmtime(fname)
                ):
