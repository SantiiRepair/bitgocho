...
class StorageWrapper:
    ...
    def hashcheckfunc(self):
        ...
        if not self.check_hashes:
            ...
            if s == self.hashes[i]:
                self._markgot(i, i)
            elif self.check_targets.get(s) and self._piecelen(i) == self._piecelen(
                self.check_targets[s][-1]
            ):
                self._markgot(self.check_targets[s].pop(), i)
                self.out_of_place += 1
            elif (
                not self.have[-1]
                and sp == self.hashes[-1]
                and (
                    i == len(self.hashes) - 1 or not self._waspre(len(self.hashes) - 1)
                )
            ):
    ...
    def _waspre(self, piece):
        return self.storage.was_preallocated(
            piece * self.piece_size, self._piecelen(piece)
        )
    ...
    def piece_came_in(self, index, begin, piece, source=None):
        ...
        length = self._piecelen(index)
        data = self.read_raw(
            self.places[index], 0, length, flush_first=self.triple_check
        )
        if data is None:
            return True
    ...
    def read_raw(self, piece, begin, length, flush_first=False):
        try:
            return self.storage.read(
                self.piece_size * piece + begin, length, flush_first
            )
        except IOError as e:
            self.failed("IO Error: " + str(e))
            return None
    ...
    def doublecheck_data(self, pieces_to_check):
        ...
        for index in sources:
            if self.have[index]:
                piece = self.read_raw(
                    self.places[index], 0, self._piecelen(index), flush_first=True
                )
                if piece is None:
                    return False
    ...
    def pickle(self):
        ...
        for p in range(len(self.hashes)):
            ...
            r.extend(pp[0])
            partials.extend([p, r])
        return {"pieces": bytes(pieces), "places": places, "partials": partials}
    ...
    def unpickle(self, data, valid_places):
        ...
        try:
            if data["pieces"] == 1:  # a seed
                ...
                _partials = [_partials[x : x + 2] for x in range(0, len(_partials), 2)]
