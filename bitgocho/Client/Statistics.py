...
class Statistics:
    ...
    def set_dirstats(self, files, piece_length):
        ...
        for i in frange:
            ...
            if l == 0:
                ...
                for piece in range(
                    int(start / piece_length), int((start + l - 1) / piece_length) + 1
                ):
                    fp.append(piece)
                    fp2.append(piece)
                start += l
    ...
    def update(self):
        ...
        if s.numPeers == 0 or self.picker.numpieces == 0:
            ...
            s.percentDone = (
                100.0
                * (float(self.picker.totalcount) / self.picker.numpieces)
                / s.numPeers
            )
        ...
        if self.filelistupdated.is_set() or self.placesopen != len(self.storage.places):
            for i, complete in enumerate(self.filecomplete):
