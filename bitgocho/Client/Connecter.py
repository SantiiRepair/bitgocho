...
class Connecter(object):
    ...
    def connection_flushed(self, connection):
        ...
        if conn.next_upload is None and (
            conn.partial_message is not None or len(conn.upload.buffer) > 0
        ):
    ...
    def got_message(self, connection, message):
        ...
        if t == CHOKE:
            ...
            c.got_request(
                piece_num,
                int.from_bytes(message[5:9], "big"),
            ...
            if i >= self.numpieces:
                ...
                connection.close()
                return
            if c.download.got_piece(
                i, int.from_bytes(message[5:9], "big"), message[9:]
            ):
                self.got_piece(i)
        else:
            connection.close()
